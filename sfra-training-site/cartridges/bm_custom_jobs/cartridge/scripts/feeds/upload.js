...
function sftpUpload(params, stepExecution) {
    ...
    var dirName = File.IMPEX + File.SEPARATOR + 'src' + File.SEPARATOR + 'CCD102';
    var exportFile = new File(dirName + File.SEPARATOR + params.ExportFileName);
    var fileName = params.AddDateExtension ? '_'+ params.ExportFileName.split('.').join('_' + new Date().toISOString().split('T')[0] + '.') : params.ExportFileName;
    ...
    try {
        ...
        if (uploadStatus.ok) {
            ...
            returnStatus = new Status(Status.ERROR, uploadStatus.status, uploadStatus.msg);
        }
    } catch (e) {
